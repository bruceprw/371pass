...
Item& Category::getItem(std::string categoryID) {
    auto foundItem = this->items.find(categoryID);
    if(foundItem != this->items.end()) {
        return this->items.find(categoryID)->second;

    }
    else {
        throw std::out_of_range("No item with ident " + categoryID + " found!");
    }
...
bool Category::deleteItem(std::string categoryID) {
    auto foundItem = this->items.find(categoryID);
    if(foundItem != this->items.end()) {
        this->items.erase(categoryID);
        return true;
    }
...
bool Category::contains(const std::string &categoryID) const {
    if (this->items.count(categoryID)) {
        return true;
    }
    else return false;
}
